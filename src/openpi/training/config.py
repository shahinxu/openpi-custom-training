...
class LeRobotCustomDataConfig(DataConfigFactory):
    ...
    # Override action key to match our dataset's "action" (singular) instead of "actions" (plural)
    action_sequence_keys: Sequence[str] = ("action",)
    
    @override
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        # Repack transform to match our dataset keys to Libero format
        # Mapping is {target_key: source_key_in_dataset}
        repack_transform = _transforms.Group(
            inputs=[
                _transforms.RepackTransform({
