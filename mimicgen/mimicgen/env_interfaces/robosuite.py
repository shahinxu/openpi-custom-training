...
class RobosuiteInterface(MG_EnvInterface):
    ...
    def target_pose_to_action(self, target_pose, relative=True):
        ...
        # Handle both regular robots with controller and FloatingHandRobot with composite_controller
        robot = self.env.robots[0]
        if hasattr(robot, 'controller'):
            controller = robot.controller
        elif hasattr(robot, 'composite_controller'):
            # For FloatingHandRobot, get the arm controller from composite_controller
            controller = robot.composite_controller.part_controllers['right']
        else:
            raise AttributeError(f"Robot {type(robot).__name__} has neither 'controller' nor 'composite_controller'")
...
class MG_HannesLift(RobosuiteInterface):
    def get_robot_eef_pose(self):
        """Get current end-effector pose for Hannes floating-hand robot.

        FloatingHandRobot exposes an ``eef_site_id`` per arm that we can use
        to read the 4x4 pose matrix directly from MuJoCo sites, avoiding
        reliance on ``robots[0].controller``.
        """

        # Handle both wrapped (robomimic EnvRobosuite) and unwrapped environments
        base_env = self.env.env if hasattr(self.env, 'env') else self.env
        robot = base_env.robots[0]
        # Assume single-arm setup and use the first arm key (e.g., "right")
        arm = robot.arms[0] if hasattr(robot, "arms") and len(robot.arms) > 0 else "right"

        site_id = robot.eef_site_id[arm]
        pos = np.array(base_env.sim.data.site_xpos[site_id])
        rot = np.array(base_env.sim.data.site_xmat[site_id].reshape(3, 3))
    ...
    def action_to_target_pose(self, action, relative=True):
        ...
        delta_position = np.asarray(action[:3])
        delta_rotation = np.asarray(action[3:6])
    ...
    def action_to_gripper_action(self, action):
        """Return gripper actuation component from action for Hannes.

        We treat the 7th dimension (index 6) as a scalar gripper command.
        """

        if len(action) > 6:
            return np.array(action[6:7])
        return np.zeros(1, dtype=np.float32)

    def get_object_poses(self):
        """Return poses of task-relevant objects.

        For the standard Lift task, the primary grasped object is ``self.cube``,
        which is a MilkObject instance whose root body tracks the object pose.
        """
        ...
        return dict(
            cube=self.get_object_pose(obj_name=base_env.cube.root_body, obj_type="body"),
        )

    def get_subtask_term_signals(self):
        """Return subtask termination signals.

        For now, we do not define intermediate subtasks for the Lift task and
        instead let the single (final) subtask terminate at the end of the
        demonstration. Configs that use this interface should therefore set
        the last subtask's ``subtask_term_signal`` to ``None``.
        """
        # No intermediate subtask termination signals; task ends with demo.
        return dict()
