...
class HannesActionWrapper:
    """
    Wrapper for Hannes environment that strips gripper action dimension.
    Hannes has an integrated gripper so it doesn't need separate gripper actions,
    but mimicgen always concatenates gripper actions. This wrapper strips the last
    dimension before passing actions to the environment.
    """
    def __init__(self, env):
        self.env = env
        
    def step(self, action):
        # Strip gripper action (last dimension) for Hannes
        if len(action) == 7:
            action = action[:6]
...
def generate_dataset(
    ...
    # Wrap Hannes environment to handle gripper action dimension
    if "Hannes" in str(env.serialize().get("env_kwargs", {}).get("robots", "")):
        print("Detected Hannes robot - wrapping environment to strip gripper action dimension")
        env = HannesActionWrapper(env)
    ...
    env_interface = make_interface(
        ...
        env=env.env if hasattr(env, 'env') else env.base_env,
    )
