...
class HannesLift_Config(MG_Config):
    """Minimal config for Hannes Lift task (single subtask).

    对应 robosuite Lift 任务，使用 MG_HannesLift 环境接口。
    这里先定义成一个简单的单子任务：相对 cube 运动，直到 episode 结束。
    """
    ...
    def task_config(self):
        self.task.task_spec.subtask_1 = dict(
            object_ref="cube",              # 来自 MG_HannesLift.get_object_poses 里的 key
            subtask_term_signal=None,        # 最后一个子任务，用 episode 结束作为结束条件
            subtask_term_offset_range=None,
            selection_strategy="random",
            selection_strategy_kwargs=None,
            action_noise=0.05,
